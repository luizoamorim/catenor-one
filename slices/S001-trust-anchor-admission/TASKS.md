...
- [ ] **T8.2 [FAST] — Key schedule + sealed context** · deps: T8.1 · sealed-context profile of PLAN §18 (HKDF-SHA256, empty salt, UTF-8 info; AES-256-GCM, 12-byte nonce, `ciphertext ‖ tag`; `aad = UTF-8(operation ‖ runId)`; `notAfter`); **nonce and ciphertext‖tag transported as base64**, decoded with a pure-JS base64 implementation (no `atob` in the runtime) — this base64 path was **not** covered by T0.7 and must pass here in simulation; `fixtures/seal-context.ts`; cross-compat test with Node `crypto` sealing · tests (simulation, production-like limits file): wrong key / other operation / other runId / flipped ciphertext / flipped tag / expired → ERROR.
