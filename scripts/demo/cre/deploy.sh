...
cre_header "deploy" "LIVE (private registry)" "registers $WORKFLOW_NAME (PAUSED)" "a workflow id (record it: configure.sh --workflow-id=…)"
...
cre "${CMD[@]}"
echo "Next: scripts/demo/cre/activate.sh --live; then configure.sh --workflow-id=<id from 'cre workflow list'> --use-deployed"
